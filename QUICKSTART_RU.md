...
# Руководство по быстрому старту ZSYS
...
async def start_handler(message):
    await message.reply("Привет! Я бот ZSYS!")
...
async def message_counter(client, message):
    # Получить счётчик сообщений для пользователя
...
- Прочитайте об [архитектуре проекта](STRUCTURE.md)
