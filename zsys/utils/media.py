...
def get_ffmpeg_paths() -> Dict[str, Optional[str]]:
    ...
    if bin_dir is None:
        ...
        return {
            exe: shutil.which(f"{exe}{exe_suffix}") for exe in executables
        }  # RU: Неизвестная ОС — только системный PATH
...
def resize_image(
    ...
    if format_to_use == "WEBP" and not features.check("webp"):
        format_to_use = (
            "PNG"  # RU: Fallback на PNG, если Pillow собран без поддержки WebP
        )
    ...
    try:
        ...
        if size2 is not None:
            ...
            )  # RU: Соотношение сторон для сохранения пропорций
...
def get_image_info(input_img: Union[str, Path, BinaryIO]) -> dict:
    """
