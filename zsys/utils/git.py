...
class GitHubAPI:
    ...
    def get_releases(self) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/releases", headers=self.headers, timeout=10
            )
    ...
    def get_latest_release(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/releases/latest", headers=self.headers, timeout=10
            )
            response.raise_for_status()
            return response.json()
        except requests.RequestException:
    ...
    def _download_asset(self, asset: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            with requests.get(
                download_url, headers=self.headers, stream=True, timeout=60
            ) as r:
                r.raise_for_status()
                with open(file_name, "wb") as f:
                    for chunk in r.iter_content(
                        chunk_size=8192
                    ):  # RU: Потоковая запись по 8 КБ — не загружает весь файл в память
                        f.write(chunk)
            return file_name
        except requests.RequestException:
            return None
