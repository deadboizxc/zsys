...
class ZsysUtils:
    ...
    def escape_html(self, text: str) -> str:
        ...
        b = text.encode()
        return _str(self._ffi, self._lib, self._lib.zsys_escape_html(b, len(b)))

    def strip_html(self, text: str) -> str:
        """Strip HTML tags and unescape basic entities."""
        b = text.encode()
        return _str(self._ffi, self._lib, self._lib.zsys_strip_html(b, len(b)))
    ...
    def truncate(self, text: str, max_chars: int, suffix: str = "…") -> str:
        ...
        return _str(
            self._ffi, self._lib, self._lib.zsys_truncate_text(b, len(b), max_chars, s)
        )
    ...
    def format_bytes(self, size: int) -> str:
        ...
        return _str(self._ffi, self._lib, self._lib.zsys_format_bytes(size))

    def format_duration(self, seconds: float) -> str:
        """Format seconds as "1h 2m 3s"."""
        return _str(self._ffi, self._lib, self._lib.zsys_format_duration(seconds))

    def human_time(self, seconds: int, short: bool = True) -> str:
        """Format seconds as Russian human time."""
        return _str(
            self._ffi, self._lib, self._lib.zsys_human_time(seconds, int(short))
        )
    ...
    def bold(self, text: str, escape: bool = True) -> str:
        b = text.encode()
        return _str(
            self._ffi, self._lib, self._lib.zsys_format_bold(b, len(b), int(escape))
        )

    def italic(self, text: str, escape: bool = True) -> str:
        b = text.encode()
        return _str(
            self._ffi, self._lib, self._lib.zsys_format_italic(b, len(b), int(escape))
        )

    def code(self, text: str, escape: bool = True) -> str:
        b = text.encode()
        return _str(
            self._ffi, self._lib, self._lib.zsys_format_code(b, len(b), int(escape))
        )

    def pre(self, text: str, lang: str = "", escape: bool = True) -> str:
        b = text.encode()
        lang_b = lang.encode()
        return _str(
            self._ffi,
    ...
    def link(self, text: str, url: str, escape: bool = True) -> str:
        ...
        return _str(
            self._ffi,
            self._lib,
            self._lib.zsys_format_link(t, len(t), u, len(u), int(escape)),
        )

    def mention(self, text: str, user_id: int, escape: bool = True) -> str:
        b = text.encode()
        return _str(
            self._ffi,
            self._lib,
            self._lib.zsys_format_mention(b, len(b), user_id, int(escape)),
        )

    def underline(self, text: str) -> str:
        b = text.encode()
        return _str(self._ffi, self._lib, self._lib.zsys_format_underline(b, len(b)))

    def strikethrough(self, text: str) -> str:
        b = text.encode()
        return _str(
            self._ffi, self._lib, self._lib.zsys_format_strikethrough(b, len(b))
        )

    def spoiler(self, text: str) -> str:
        b = text.encode()
        return _str(self._ffi, self._lib, self._lib.zsys_format_spoiler(b, len(b)))

    def quote(self, text: str) -> str:
        b = text.encode()
        return _str(self._ffi, self._lib, self._lib.zsys_format_quote(b, len(b)))
