...
class Database(AbstractContextManager, MutableMapping, ABC):
    ...
    def migrate(self, old_db: 'Database') -> None:
        ...
        try:
            ...
            raise MigrationError(f"Ошибка миграции данных: {e}") from e  # RU: Оборачиваем исходное исключение.
...
class MemoryStorage(BaseStorage):
    ...
    def _cleanup_expired(self) -> None:
        ...
        expired_keys = [
            ...
            if expiry_time <= now  # RU: Ключ истёк если текущее время >= времени истечения.
        ]
    ...
    async def set(
        ...
        if expire is not None:
            self._expiry[key] = time.time() + expire  # RU: Вычисляем абсолютный timestamp истечения.
