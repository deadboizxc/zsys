...
class MediaConfig(BaseConfig):
    ...
    storage_backend: str = Field(
        default="LOCAL", description="Storage backend (LOCAL, S3, CLOUDFLARE, etc.)"
    )
    ...
    # === CDN ===
    cdn_url: str = Field(default="", description="Public CDN URL")

    # === Limits ===
    max_file_size: int = Field(
        default=50 * 1024 * 1024, description="Maximum file size (bytes)"
    )
    max_total_storage: int = Field(
        default=10 * 1024 * 1024 * 1024, description="Maximum total storage (bytes)"
    )
    ...
    # === Processing ===
    auto_convert: bool = Field(default=True, description="Auto-convert to web formats")
    generate_thumbnails: bool = Field(default=True, description="Generate thumbnails")

    # === Image processing ===
    max_image_dimension: int = Field(
        default=4096, description="Maximum image dimension (pixels)"
    )
...
class BaseMediaService(BaseClient[MediaConfig]):
    ...
    @abstractmethod
    async def download(
        self, file_id: str, destination: Union[str, Path, BinaryIO] = None, **kwargs
    ) -> Union[bytes, str]:
        """
    ...
    async def convert(
        self, file_id: str, target_format: str, **kwargs
    ...
    async def resize_image(
        self, file_id: str, width: int = None, height: int = None, **kwargs
    ) -> Optional[UploadResult]:
        """
        Resize image.
    ...
    async def get_storage_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            if self._config.max_total_storage > 0
            else 0,
        }
