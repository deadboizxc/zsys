...
async def interact_with(
    message: "Message", timeout: int = 5, check_self: bool = True
) -> "Message":
    """
...
async def wait_for_reply(
    client, chat_id: int, message_id: int, timeout: int = 30, poll_interval: float = 0.5
) -> Optional["Message"]:
    """
    ...
    while elapsed < timeout:
        async for msg in client.get_chat_history(chat_id=chat_id, limit=10):
            if (
                hasattr(msg, "reply_to_message_id")
                and msg.reply_to_message_id == message_id
            ):
                return msg
